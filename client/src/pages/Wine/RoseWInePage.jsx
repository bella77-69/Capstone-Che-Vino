  ...
  useEffect(() => {
    const fetchWineData = async () => {
      try {
        ...
        const roseWines = fetchedData.filter((wine) => wine.type === "rose");
        setData(roseWines);

        if (roseWines.length > 0) {
          const randomIndex = Math.floor(Math.random() * roseWines.length);
          setInitialWine(roseWines[randomIndex]);
        }
      } catch (err) {
        console.log(err);
      }
    ...
    <section className="wines-section">
      <div className="wines-container">
        ...
        <header className="wines-section-header">
          <h1 className="wines-section-title">
            {" "}
